...
export default function TableReservation({ onClose, onSuccess }) {
  ...
  const isFormInvalid = useMemo(() => {
    ...
    return (
      !firstName.trim() || !lastName.trim() || !resDate || !startTime || !endTime || 
      !hasOneHourDiff || !guestCount || guestCount <= 0 || !selectedMunicipality || 
      !selectedBarangay || isOtherAllergyEmpty || !receipt
    );
          ...
          <div className="res-panel fade-in">
            ...
            <div className="res-form">
              ...
              <div className="input-group">
                ...
                {allergy === "Other" && (
                  <input type="text" className="res-input fade-in" style={{ marginTop: "10px" }} placeholder="Specify allergy" value={otherAllergy} onChange={(e) => setOtherAllergy(e.target.value)} />
                )}
              </div>
              ...
              <div className="input-group">
                <label>Upload Receipt (Gcash/Maya)</label>
                <input type="file" ref={fileInputRef} style={{ display: "none" }} accept="image/*,.pdf" onChange={(e) => setReceipt(e.target.files[0])} />
                <button type="button" className="btn-link-mode" style={{ width: "100%", marginTop: "5px" }} onClick={() => fileInputRef.current.click()}>
