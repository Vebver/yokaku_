...
export default function TableReservation({ onClose, onSuccess }) {
  ...
  useEffect(() => {
    const fetchTableSchedule = async () => {
      if (selectedId && resDate) {
        try {
          const res = await axios.get(`http://localhost:5000/api/reservations/table-schedule`, {
  ...
  const orderSummary = useMemo(() => {
    const totalOrderPrice = selectedItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
  ...
  const isFormInvalid = useMemo(() => {
    ...
    return (
      !firstName.trim() || !lastName.trim() || !email.trim() || !resDate || !startTime || !endTime ||
      !hasOneHourDiff || !guestCount || guestCount <= 0 || !selectedMunicipality || !selectedBarangay ||
      isOtherAllergyEmpty || selectedItems.length === 0 || hasConflict
    );
  }, [firstName, lastName, email, resDate, startTime, endTime, guestCount, selectedMunicipality, selectedBarangay, allergy, otherAllergy, selectedItems, tableSchedule]);
  ...
  const fullReservationData = useMemo(() => {
    const muniName = municipalities.find((m) => m.code === selectedMunicipality)?.name || "";
    const brgyName = barangays.find((b) => b.code === selectedBarangay)?.name || "";
    return {
      firstName, lastName, email, phone, municipality: muniName, barangay: brgyName, guestCount, resDate,
      startTime, endTime, tableLabel: primaryTable?.label, linkedTables: linkedIds,
      allergy: allergy === "Other" ? otherAllergy : allergy, packages: selectedItems,
    };
  }, [firstName, lastName, email, phone, selectedMunicipality, selectedBarangay, guestCount, resDate, startTime, endTime, primaryTable, linkedIds, municipalities, barangays, allergy, otherAllergy, selectedItems]);
          ...
          <div className="map-container">
            ...
            <div className="tables-area">
              {TABLES_DATA.map((table) => {
                ...
                if (dbStatus === "Confirmed" || dbStatus === "Seated") statusClass = "occupied";
                ...
                return (
                  <div key={table.id} className={`floor-table ${table.type} ${statusClass}`} style={{ top: table.top, left: table.left }} onClick={() => handleTableClick(table)}>
                    {renderChairs(table)}
      ...
      <aside className="floor-sidebar" onClick={(e) => e.stopPropagation()}>
        {hasActiveReservation ? (
          <div className="reserved-notice fade-in">
            <span className="info-icon-large"><Info size={32} color="#e74c3c" /></span>
          ...
          <div className="res-panel fade-in">
            ...
            <div className="table-schedule-section" style={{ padding: '10px', backgroundColor: '#f9f9f9', borderRadius: '8px', marginBottom: '15px', border: '1px solid #eee' }}>
  <h4 style={{ fontSize: '12px', marginBottom: '8px', color: '#555', display: 'flex', alignItems: 'center', gap: '5px' }}>
    <Clock size={14} /> Occupied Slots for {resDate || 'selected date'}
  </h4>
  {tableSchedule.length > 0 ? (
    <div className="schedule-list">
      {tableSchedule.map((res, index) => (
        <div key={index} style={{ fontSize: '12px', padding: '8px', background: '#fff', borderLeft: '3px solid #f38d31', marginBottom: '4px', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
          {/* Removed Customer Name - Only displaying the Time Range */}
          <div style={{ fontWeight: '600', color: '#333' }}>
            <Clock size={10} style={{ marginRight: '4px' }} />
            {res.startTime} - {res.endTime}
          </div>
          <div style={{ fontSize: '10px', color: '#888' }}>Status: {res.status}</div>
        </div>
      ))}
    </div>
  ) : (
    <p style={{ fontSize: '11px', color: '#2a9d8f' }}>No reservations. All slots are available.</p>
  )}
</div>
            ...
            <div className="res-form">
              ...
              <div className="input-group">
                ...
                <input type="date" value={resDate} min={todayStr} onChange={(e) => { setResDate(e.target.value); setStartTime(""); setEndTime(""); }} />
              </div>
              ...
              <div className="input-row">
                <div className="input-group">
                  ...
                  <select className="res-input-dropdown" value={startTime} onChange={(e) => { setStartTime(e.target.value); setEndTime(""); }}>
                    <option value="">--:-- --</option>
                    {availableStartTimeOptions.map((t) => <option key={`start-${t}`} value={t}>{t}</option>)}
                  </select>
                </div>
                <div className="input-group">
              ...
              <div className="input-group">
                ...
                {allergy === "Other" && (
                  <input type="text" className="res-input fade-in" style={{ marginTop: "10px", borderColor: "#f38d31" }} placeholder="Specify allergy" value={otherAllergy} onChange={(e) => setOtherAllergy(e.target.value)} />
                )}
              </div>

              <div className="input-group">
                <label>PACKAGES WE'RE OFFERING</label>
                <button type="button" className="btn-link-mode" style={{ width: "100%", marginBottom: "10px" }} onClick={() => setIsMenuOpen(true)}>
                  <Layers size={16} /> {selectedItems.length > 0 ? `Selected ${selectedItems.length} Items` : "View Packages"}
                </button>
              </div>
      ...
      <MenuModal isOpen={isMenuOpen} onClose={() => setIsMenuOpen(false)} onSelectedItemsChange={(items) => setSelectedItems(items)} initialSelectedItems={selectedItems} />
      <ReservationSummary isOpen={isSummaryOpen} onClose={() => setIsSummaryOpen(false)} orderSummary={orderSummary} reservationData={fullReservationData} onConfirm={handleConfirmReservation} loading={loading} />
    </div>
  );
}