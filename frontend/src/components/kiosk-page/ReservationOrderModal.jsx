...
const ReservationOrderModal = ({ isOpen, onClose, item, onAdd, allProducts }) => {
  ...
  return (
    <div className={`res-modal-overlay ${isOpen ? "active" : ""}`} onClick={onClose}>
      {/* Added 'dynamic-height' class based on content */}
      <div className={`res-bottom-sheet ${isOpen ? "slide-up" : ""} ${isBundle ? 'is-bundle' : 'is-single'}`} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-container">
              ...
              <div className="bundle-options">
                ...
                <div className="chip-grid">
                  {chickenOptions.map((f) => (
                    <button 
                      key={f.id} 
                      className={`selection-chip ${customizations.flavor === f.name ? 'active' : ''}`}
                ...
                /* Extra padding for single items to make it look better */
                <div className="single-item-spacer"></div>
            )}
          ...
          <div className="fixed-footer">
            ...
            <button 
              className={`btn-submit ${isBundle && (!customizations.flavor || !customizations.drink) ? 'disabled' : ''}`} 
              onClick={handleAdd}
            >
      ...
      <style jsx="true">{`
        ...
        .res-bottom-sheet.is-bundle {
          height: 85vh; /* Large sheet for bundles */
        }

        .res-bottom-sheet.is-single {
          height: auto; /* Shrink to fit for single items */
          max-height: 85vh;
        }
        ...
        .single-item-spacer { height: 40px; }
        ...
        .item-price-tag { color: #ffcc00; font-size: 1.5rem; font-weight: 800; margin: 5px 0; }
        .item-description { color: #888; font-size: 0.95rem; line-height: 1.4; }
        ...
        .selection-chip { 
          background: #f4f4f4; border: 2px solid transparent; padding: 14px; 
          border-radius: 15px; font-weight: 700; color: #444; cursor: pointer;
        ...
        .qty-label { font-size: 0.75rem; font-weight: 800; color: #bbb; }
        ...
        .qty-picker button { background: #fff; border: 1px solid #eee; width: 44px; height: 44px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; }
        .qty-val { font-size: 1.6rem; font-weight: 900; min-width: 30px; text-align: center; color: #222; }
        ...
        .btn-cancel { background: #fff; border: 2px solid #eee; padding: 16px 25px; border-radius: 20px; font-weight: 800; cursor: pointer; color: #aaa; transition: 0.2s; }
        .btn-cancel:hover { background: #fafafa; color: #666; }
        .btn-submit { 
          flex: 1; background: #ffcc00; color: #000; border: none; 
          padding: 18px; border-radius: 20px; font-weight: 800; font-size: 1.1rem; cursor: pointer;
          box-shadow: 0 4px 15px rgba(255, 204, 0, 0.2);
        }
        .btn-submit.disabled { background: #f0f0f0; color: #ccc; cursor: not-allowed; box-shadow: none; }
      `}</style>
    </div>
  );
};
