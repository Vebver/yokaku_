...
import {
  ...
  Soup, Salad, Clock, Receipt, UtensilsCrossed, RefreshCw, Banknote, CreditCard
} from "lucide-react";
      ...
      <div className="kiosk-timer-wrapper" style={{ zIndex: 5000 }}>
        <div className="header-id-section" style={{ background: "#222", border: "2px solid #ffcc00", padding: "10px 15px", borderRadius: "10px" }}>
          ...
          <div className="id-details" style={{ marginLeft: "10px" }}>
            ...
            <span className="id-value" style={{ color: "#fff", fontWeight: "900", display: "block", fontSize: "15px" }}>
              {localStorage.getItem(SAVED_TABLE_ID) === "takeout" ? "TAKE-OUT" : localStorage.getItem(SAVED_TABLE_ID) ? `TABLE ${localStorage.getItem(SAVED_TABLE_ID)}` : "WALK-IN GUEST"}
            </span>
          </div>
        </div>
        ...
        {isTimerRunning && hasActiveBundle && (
          <div className="timer-box" onClick={() => setShowSessionModal(true)} style={{ cursor: "pointer", border: "2px solid #ffcc00", marginLeft: "auto" }}>
            ...
            <button className="finish-session-header-btn" onClick={(e) => { e.stopPropagation(); handleFinishClick(); }}>FINISH</button>
          </div>
        )}
      </div>
    ...
    return (
        ...
        <span>₱{(p * q).toFixed(2)}</span>
            ...
            {isFinalCheckout ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                 <p style={{color: '#aaa', fontSize: '0.8rem'}}>Please proceed to the counter to settle your bill.</p>
                 <button className="res-modal-btn-primary" onClick={handleEndSession}>FINISH SESSION</button>
                 <button className="res-btn-cancel" onClick={() => setShowBillInfo(false)}>BACK</button>
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                <button className="res-modal-btn-primary" onClick={() => confirmPaymentChoice("Pay Now")}><Banknote size={18} /> PAY NOW (At Counter)</button>
                <button className="res-modal-btn-primary" style={{background: "#444", border: 'none'}} onClick={() => confirmPaymentChoice("Pay Later")}><Clock size={18} /> PAY LATER</button>
      ...
      {showFlavorModal && (
        <div className="res-modal-overlay" style={{ zIndex: 9000 }}>
          <div className="res-modal-card" style={{ maxWidth: "600px", width: "95%" }}>
            <h2 style={{ color: "#ffcc00", textAlign: "center" }}>
              {selectedItem?.name.toLowerCase().includes("ramen") ? "Ramen Choice" : "Unlimited Wings"}
            </h2>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginTop: '20px' }}>
              {(selectedItem?.name.toLowerCase().includes("ramen") ? dynamicRamenFlavors : dynamicFlavors).map((f) => (
                <button
                  ...
                  style={{
                    padding: "15px 10px", borderRadius: "10px", border: "1px solid #ffcc00",
                    background: selectedFlavors.includes(f) ? "#ffcc00" : "none",
                    color: selectedFlavors.includes(f) ? "#000" : "#fff", fontWeight: "bold"
                  }}
                >
                  {f}
                </button>
            ...
            <div style={{ display: "flex", gap: "15px", marginTop: "30px" }}>
              ...
              <button className="res-modal-btn-primary" style={{ flex: 2 }} onClick={confirmFlavors}>
      ...
      {showSessionModal && (
        <div className="res-modal-overlay" style={{ zIndex: 7000 }}>
          <div className="res-modal-card" style={{ textAlign: "center", padding: "40px" }}>
            ...
            <h2 style={{ color: "#ffcc00", marginBottom: "10px" }}>{hasActiveBundle ? "SESSION ACTIVE" : "ORDER PLACED"}</h2>
            {hasActiveBundle && <div style={{ fontSize: "3rem", fontWeight: "900", color: "#fff", margin: "20px 0" }}>{formatTime(timeLeft)}</div>}
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {hasActiveBundle && <button className="res-modal-btn-primary" style={{ background: "#28a745" }} onClick={() => {setSelectedItem({id: localStorage.getItem("kiosk_active_bundle_id"), name: 'Unlimited'}); setIsRefillMode(true); setShowFlavorModal(true); setShowSessionModal(false);}}><RefreshCw size={18} /> REFILL CHICKEN</button>}
