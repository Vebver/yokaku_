  ...
  try {
    // 1. Add items to kitchen/order list
    ...
    // 3. ONLY SYNC BILLING IF PAYING NOW
    if (isPayNow) {
      const currentTotal = calculateTotal(); // Get total of everything so far
      await syncWithDashboard(dynamicResId, currentTotal, "Cash", "verified");
  ...
  const handleHeaderPayClick = async () => {
    setIsPaymentProcessing(false); // Reset state before opening
          ...
          {isTimerRunning && (
            ...
            >
              ...
              <span
                ...
                style={{ color: "#fff", fontWeight: "900", fontSize: "2.5rem" }}
              >
                {formatTime(timeLeft)}
              </span>
            </div>
          ...
          <div className="res-modal-overlay" style={{ zIndex: 10000 }}>
            ...
            >
              ...
              >
                ...
                >
                  <Clock size={18} className="me-2" /> PAY LATER
                </button>
              </div>
            </div>
          </div>
              ...
              >
                {isFinalCheckout ? (
                  <>
                    <button
                      className="res-modal-btn-primary"
                      // FIX: Disable if already verified or currently processing
                      disabled={
                        isPaymentProcessing ||
                        storage.getItem(PAYMENT_CHOICE_KEY) === "verified"
                      }
                      onClick={async () => {
                        if (isPaymentProcessing) return; // Guard
                        ...
                        try {
                          ...
                          // CRUCIAL: Mark as verified so the button stays disabled
                          storage.setItem(PAYMENT_CHOICE_KEY, "verified");
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (selectedFlavors.includes(f))
                        ...
                        if (selectedItem.name.toLowerCase().includes("ramen"))
