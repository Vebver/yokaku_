    ...
    const fetchMenu = async () => {
      try {
        ...
        const grouped = data.reduce((acc, item) => {
          ...
          if (!acc[cat]) acc[cat] = [];
          const fullImage = item.image_url?.startsWith("http")
            ? item.image_url
      ...
      timerRef.current = setInterval(() => {
        ...
        if (remaining === 1800) {
          ...
          audioRef.current
            ...
            .catch((e) =>
              console.error(
                "Audio blocked by browser. User must interact with screen first.",
                e,
              ),
            );
        }
...
const confirmFlavors = async () => {
  ...
  const customization = isRefillMode 
    ...
    : `Flavors: ${selectedFlavors.join(", ")} | Drink: ${selectedDrink}`;
  ...
  if (isRefillMode) {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE}/orders/place`, {
        ...
        items: [{
          ...
          price: 0,
          is_refill: true // Sends the signal to the backend check
        }]
      });
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        fetchCurrentBill(); // Refresh the bill UI
      }
    } catch (err) {
      if (err.response && err.response.status === 429) {
        // This is our 15-minute cooldown message!
    ...
    // Normal bundle ordering (adds to tray/cart, not DB yet)
 ...
 const handleRefillClick = async () => {
  ...
  if (bundle) {
    // Just set the state and open the modal
    setSelectedItem({
      id: bundle.item_id || bundle.id,
      name: bundle.name || bundle.item_name,
    });
    ...
    alert("Please order a Hangout Bundle first.");
  }
};
  ...
  const submitOrderToDatabase = async (
    ...
    try {
      ...
      if (response.ok) {
        ...
        const newItems = itemsToSubmit.map((i) => ({
          name: i.name,
          price: i.price || 0,
          quantity: i.quantity,
          customizations: i.customizations,
      ...
      <div className="kiosk-timer-wrapper" style={{ zIndex: 5000 }}>
        ...
        {isTimerRunning && (
          <div
            ...
            style={{ cursor: "pointer", border: "2px solid #ffcc00" }}
          >
        ...
        <main className="res-content-area">
          <div className="res-grid-container">
            {(menuData[activeCategory] || []).map((item) => {
              if (hasActiveBundle && item.name.toLowerCase().includes("bundle"))
                return null;
              return (
                <div
                  key={item.id}
        ...
        <div className="res-modal-overlay" style={{ zIndex: 9000 }}>
          ...
          >
            <h2 style={{ color: "#ffcc00", textAlign: "center" }}>
              {isRefillMode ? "Refill Selection" : "Bundle Setup"}
            </h2>
            <h4 style={{ color: "#fff", margin: "20px 0 10px" }}>
              Select Chicken Flavors (Max 4)
            </h4>
            <div
              style={{
                display: "grid",
        ...
        <div className="res-modal-overlay" style={{ zIndex: 7000 }}>
          ...
          >
            ...
            >
              ...
              >
                <RefreshCw size={18} style={{ marginRight: "8px" }} /> REFILL
                CHICKEN
              </button>
              <button
                className="res-modal-btn-primary"
