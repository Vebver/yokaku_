...
function Reports() {
  ...
  const comparisonData = {
    ...
    datasets: [
      { label: 'Revenue', data: [4500, 5200, 4800, 7100, 6800, 8200], backgroundColor: '#0d6efd', borderRadius: 5 },
      { label: 'Expenses', data: [3100, 3800, 3200, 4100, 3900, 4800], backgroundColor: '#e9ecef', borderRadius: 5 },
    ],
  };
    ...
    <div className="container-fluid fade-in pb-5">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted">Analyze business growth and revenue</p>
        </div>
        <div className="d-flex gap-2">
          <button className="btn btn-outline-secondary shadow-sm"><i className="bi bi-download me-2"></i>CSV</button>
          <button className="btn btn-primary shadow-sm"><i className="bi bi-printer me-2"></i>PDF</button>
        </div>
      </div>

      {/* Row 1: Comparison & Breakdown */}
      <div className="row g-4 mb-4">
        <div className="col-lg-8">
          <div className="card border-0 shadow-sm p-4 h-100">
            <h5 className="fw-bold mb-4">Revenue vs Expenses</h5>
        ...
        <div className="col-lg-4">
          <div className="card border-0 shadow-sm p-4 h-100">
            ...
            {/* Static breakdown content */}
            {['Inventory', 'Staffing', 'Marketing'].map((item, i) => (
              <div key={i} className="mb-3">
                <div className="d-flex justify-content-between mb-1 small fw-bold">
                  <span>{item}</span>
                  <span className="text-muted">{100 - (i * 30)}%</span>
                </div>
                <div className="progress" style={{ height: '6px' }}>
                  <div className="progress-bar bg-primary" style={{ width: `${100 - (i * 30)}%` }}></div>
                </div>
              </div>
            ))}
          </div>
