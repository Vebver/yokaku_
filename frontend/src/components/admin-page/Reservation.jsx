...
// --- SUB-COMPONENT: REAL-TIME COUNTDOWN (Remains unchanged) ---
  ...
  const fetchReservations = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE}/reservations`, {
        headers: { Authorization: `Bearer ${token}` }, // 2. ATTACH TOKEN
      });
            ...
            <>
              ...
              <div className="card border-0 shadow-sm mb-3">
                <div className="card-body">
                  ...
                  {loadingItems ? <div className="text-center py-3"><div className="spinner-border spinner-border-sm"></div></div> : (
                    <div className="item-list">
                      {orderItems.length > 0 ? orderItems.map((order, idx) => {
                          let customs = null;
                          if (order.customizations) {
                            try { customs = typeof order.customizations === "string" ? JSON.parse(order.customizations) : order.customizations; } catch(e) { customs = null; }
                          }
                          return (
                            <div key={idx} className="mb-3 pb-2 border-bottom border-light">
                              ...
                              {customs && (
                                <div className="mt-1 ps-2 border-start border-2 border-warning-subtle small text-muted">
                                  {customs.flavor && <div>Flavor: <span className="text-dark fw-bold">{customs.flavor}</span></div>}
                                  {customs.drink && <div>Drink: <span className="text-dark fw-bold">{customs.drink}</span></div>}
                                  {customs.spiceLevel && <div>Spice: <span className="text-dark fw-bold">{customs.spiceLevel}</span></div>}
                                </div>
                              )}
                            </div>
                          );
                  ...
                  <div className="d-flex justify-content-between align-items-center">
                    <div>
                      ...
                      <div className="fs-4 fw-bold">
                        ₱{selectedRes.amount > 0 ? Number(selectedRes.amount).toLocaleString(undefined, { minimumFractionDigits: 2 }) : calculateDrawerTotal().toLocaleString(undefined, { minimumFractionDigits: 2 })}
                      </div>
                    </div>
