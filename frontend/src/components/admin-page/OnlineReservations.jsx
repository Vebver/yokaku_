...
const OnlineReservations = () => {
  ...
  const fetchReservations = async () => {
    try {
      ...
      const filtered = response.data.filter(item => !item.reservation_id?.includes("WALK"));
  ...
  const currentItems = inquiries.slice((currentPage - 1) * itemsPerPage, indexOfLastItem);
  const totalPages = Math.ceil(inquiries.length / itemsPerPage);

  if (loading) return <div className="p-5 text-center text-muted"><div className="spinner-border text-primary"></div></div>;
    ...
    <div className="container-fluid py-4 bg-light min-vh-100">
      ...
      <div className="row align-items-center mb-4 px-2">
        <div className="col">
          ...
          <p className="text-muted small mb-0">Manage website bookings and customer downpayments</p>
        </div>
        <div className="col-auto">
          <div className="bg-white border rounded-pill px-3 py-1 shadow-sm small fw-bold">{inquiries.length} Bookings</div>
        </div>
      </div>
              ...
              {currentItems.map((item) => (
                <tr key={item.reservation_id}>
                  <td className="ps-4">
                    ...
                    <code className="x-small text-muted">{item.reservation_id}</code>
                  </td>
                  <td>
                    <Armchair size={14} className="me-1 text-muted" />
                    <span className="small fw-bold">{item.assigned_tables || "T-?"}</span>
                  </td>
                  <td>
                    <div className="fw-bold small">{new Date(item.reservation_date).toLocaleDateString()}</div>
                    <div className="x-small text-muted">{item.reservation_time}</div>
                  </td>
                  <td>
                    <span className={`badge border px-2 py-1 small fw-normal ${
                      item.payment_status?.toLowerCase() === 'verified' ? 'bg-success-subtle text-success border-success-subtle' : 'bg-warning-subtle text-warning border-warning-subtle'
                    }`}>
      ...
      <div className="mt-4 px-3 d-flex justify-content-between align-items-center">
        <span className="small text-muted">Showing {currentItems.length} of {inquiries.length} items</span>
        <div className="btn-group shadow-sm bg-white rounded border">
          ...
          <span className="btn btn-sm disabled border-0 px-3 text-dark fw-bold">Page {currentPage}</span>
      ...
      <div className="offcanvas offcanvas-end border-0 shadow-sm" tabIndex="-1" id="onlineDrawer" data-bs-backdrop="false" style={{ width: "min(100%, 450px)" }}>
          ...
          {selectedRes && (
            <div className="d-flex flex-column h-100">
              ...
              <div className="p-3 border-bottom bg-light-subtle">
                <div className="d-flex align-items-center gap-3 mb-2">
                  ...
                  <div className="overflow-hidden">
                    ...
                    <div className="x-small text-muted text-truncate">{selectedRes.email}</div>
                  </div>
                </div>
              ...
              <div className="p-3 border-bottom">
                ...
                {selectedRes.allergies && (
                  <div className="p-2 bg-warning-subtle rounded border border-warning-subtle d-flex gap-2">
                    <Info size={14} className="text-warning mt-1 flex-shrink-0" />
              ...
              <div className="p-3 flex-grow-1 overflow-auto">
                <span className="x-small fw-bold text-muted text-uppercase d-block mb-2">Orders</span>
                {loadingItems ? (
              ...
              <div className="p-3 bg-dark text-white sticky-bottom">
                <div className="d-flex justify-content-between align-items-center mb-3">
                  ...
                  <span className={`badge py-2 px-3 ${getStatusBadge(selectedRes.status)}`}>
                    {selectedRes.status?.toUpperCase()}
                  </span>
                </div>
                <button className="btn btn-dark btn-sm w-100 fw-bold border border-white border-opacity-25 py-2" data-bs-dismiss="offcanvas">
                  Close Details
                </button>
              </div>
