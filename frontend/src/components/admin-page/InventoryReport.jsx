...
import { AlertCircle, Activity, Package } from 'lucide-react';
  ...
  return (
    <div className="row g-3 g-md-4 mt-2">
      <div className="col-12">
        <h4 className="fw-bold mb-1 fs-3 fs-md-4">Inventory Report</h4>
        <p className="text-muted small">Real-time stock & consumption tracking</p>
      </div>
              ...
              <table className="table align-middle mb-0">
                <thead className="table-light">
                  <tr className="x-small text-uppercase text-muted">
                    <th className="ps-0 border-0">Ingredient</th>
                    {/* Hide "Starting" on mobile to save space */}
                    <th className="border-0 d-none d-lg-table-cell">Starting</th>
                    <th className="border-0">Used</th>
                    <th className="border-0">Ending</th>
                    <th className="text-end pe-0 border-0">Value</th>
                  </tr>
                </thead>
                <tbody>
                  {usageData.length > 0 ? usageData.map((item, idx) => (
                    <tr key={idx}>
                      <td className="ps-0 py-3 fw-bold text-dark small">{item.name}</td>
                      ...
                      <td className="fw-bold text-end pe-0 text-dark small">{formatCurrency(item.inventory_value)}</td>
                    </tr>
                  )) : (
