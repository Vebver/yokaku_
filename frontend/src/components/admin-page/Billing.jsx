                ...
                <tr key={p.payment_id}>
                  <td className="ps-4 py-3">
                    <div className="fw-bold">
                      {p.first_name === "Walk-in" ? `Kiosk (Table ${p.table_number || '?'})` : `${p.first_name || 'Guest'} ${p.last_name || ''}`}
                    </div>
                  ...
                  <td className="fw-bold text-primary">
                    {p.amount && Number(p.amount) > 0 ? `₱${Number(p.amount).toLocaleString()}` : "₱0.00"}
                  </td>
                  <td>
                    ...
                    {orderItems.map((item, idx) => (
                      <div key={idx} className="mb-3 pb-2 border-bottom border-light">
                        <div className="d-flex justify-content-between small">
                          <div className="fw-bold">
                            {item.name || item.item_name} <span className="text-muted ms-1">x{item.quantity}</span>
                            {item.customizations?.includes("[TAKE-OUT]") && (
                              <span className="badge bg-warning text-dark ms-2" style={{ fontSize: "0.6rem" }}>TO-GO</span>
                            )}
                          </div>
                          <div className="fw-bold text-dark">₱{(item.quantity * item.price).toLocaleString()}</div>
                        </div>
                        {item.customizations && (
                          <div className="text-muted" style={{ fontSize: "0.7rem" }}>
                            {item.customizations.replace("[TAKE-OUT]", "").trim()}
                          </div>
                        )}
                      </div>
                    ))}
                    ...
                    <div className="d-flex flex-column gap-3">
                      ...
                      {dpAmount > 0 && (
                        <div className="d-flex justify-content-between border-bottom border-secondary pb-2 text-info">
                          <span className="small">{isWalkIn ? "Paid at Kiosk" : "Downpayment"}</span>
                      ...
                      <div className="d-flex justify-content-between align-items-end pt-1 text-warning">
                        <div>
                          <span className="small text-uppercase fw-bold opacity-75">
                            {balanceToPay === 0 ? "Account Cleared" : "Remaining Balance"}
                          </span>
                          <h1 className="fw-bold mb-0">₱{balanceToPay.toLocaleString()}</h1>
                        </div>
                        <span className={`badge ${balanceToPay === 0 ? "bg-success" : "bg-primary"}`}>{selectedPayment.status}</span>
                      </div>
                      {selectedPayment.status !== "completed" && balanceToPay > 0 ? (
                        ...
                        <div className="alert alert-success py-3 text-center mb-0 mt-2 fw-bold">TRANSACTION SETTLED</div>
                      )}
                    </div>
                  );
                })()}
                <button className="btn btn-link text-white-50 w-100 mt-3 text-decoration-none" data-bs-dismiss="offcanvas">Close</button>
              </div>
            </>
          )}
        </div>
